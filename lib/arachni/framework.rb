...
class Framework
    ...
    def initialize( opts = Arachni::Options.instance, &block )
        ...
        @page_queue = state.framework.page_queue

        # Holds paths to be audited.
        @url_queue = state.framework.url_queue
    ...
    #
    # @param   [Block]  block
    #   A block to call after the audit has finished but before {#clean_up}.
    def run( &block )
        prepare
    ...
    def clean_up( shutdown_browser_cluster = true )
        ...
        state.framework.update_browser_skip_states browser_job_skip_states
    ...
    def browser_job_skip_states
        return [] if !@browser_cluster
        browser_cluster.skip_states_for( browser_job.id ).collection
    end
