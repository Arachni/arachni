    ...
    trace: function ( depth_offset ) {
        ...
        while( stackArrayOffset <= error.stackArray.length - 1 ) {
            ...
            if( !_token.has_function( f ) ) {
                ...
                if( f ) {
                    trace_data.source    = f;
                    trace_data.arguments = f.arguments
                }

                var stack = error.stackArray[stackArrayOffset];
                if( stack.line ) trace_data.line = stack.line;
...
//                trace_data.error = error;
    ...
    has_function: function( func ) {
        // Traced functions are dynamic and can't be compared using === so let's do
        // a quick check and dirty check to get this special case over with.
        ...
        for( var name in namespace ){
            ...
            if( Object.prototype.toString.call(potentialFunction) === '[object Function]' ){
                ...
                if( !_token.traced[namespace_function_name] ) {
                    namespace[name] = _token.get_traced_function( potentialFunction );
    ...
    add_trace_to_prototypes: function () {
        _token.add_trace_to_namespace( location.prototype );
        _token.add_trace_to_namespace( String.prototype );
        _token.add_trace_to_namespace( HTMLDocument.prototype );
        _token.add_trace_to_namespace( HTMLBodyElement.prototype );
        _token.add_trace_to_namespace( Element.prototype );
    },
