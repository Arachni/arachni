...
var _tokenDOMMonitor = _tokenDOMMonitor || {
    ...
    elements_with_events: function ( offset, batch_size ) {
        ...
        for( var i = 0; i < length; i++ ) {
            ...
            if( relevant_element_index <= offset ) continue;

            events_with_elements.push( e );

            // Batch size reached, send it on its way.
            if( events_with_elements.length == batch_size )
    ...
    registerEvent: function ( element, event, handler ) {
        ...
        // bail out.
        if( element.tagName && !_tokenDOMMonitor.is_valid_event_for_element(
