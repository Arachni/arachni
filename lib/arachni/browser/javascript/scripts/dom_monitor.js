...
var _tokenDOMMonitor = _tokenDOMMonitor || {
    ...
    bequeath_events: function( element ) {
        if( !element['_arachni_events'] || !element.hasChildNodes() ) return;
        ...
        for( var i = 0; i < children.length; i++ ) {
            ...
            if( !('_arachni_events' in child) ) child['_arachni_events'] = [];

            child['_arachni_events'] =
                element['_arachni_events'].concat( child['_arachni_events'] );

            child['_arachni_events'] =
                _tokenDOMMonitor.arrayUnique( child['_arachni_events'] )
        }
    },
