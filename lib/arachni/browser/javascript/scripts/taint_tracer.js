...
var _tokenTaintTracer = _tokenTaintTracer || {
    ...
    // Data-flow (find_and_log_taint() -- log_data_flow_sink()) sink.
    data_flow_sink:      [],
    ...
    data_flow_sinks_to_monitor: {
        ...
        once: [
            ...
            [
                window,
                ['eval', 'encodeURIComponent', 'decodeURIComponent',
                'encodeURI', 'decodeURI']
            ],
            ...
            [String.prototype, ['replace', 'concat']], // This may be pushing it.
    ...
    // Initialize.
    initialize: function ( taint ) {
        if( _tokenTaintTracer.initialized ) return;

        _tokenTaintTracer.taint = taint;
    ...
    log_data_flow_sink: function ( frame_data ){
        _tokenTaintTracer.data_flow_sink.push({
            data:  frame_data,
            trace: _tokenTaintTracer.trace()
        })
    },
    ...
    log_execution_flow_sink: function (){
        _tokenTaintTracer.execution_flow_sink.push({
            data:  arguments,
            trace: _tokenTaintTracer.trace()
        })
    },

    flush_execution_flow_sink: function (){
        var a = _tokenTaintTracer.execution_flow_sink.slice();
        _tokenTaintTracer.execution_flow_sink = [];
        return a;
    },

    flush_data_flow_sink: function (){
        var a = _tokenTaintTracer.data_flow_sink.slice();
        _tokenTaintTracer.data_flow_sink = [];
        return a;
    },
        ...
        while( stackArrayOffset <= error.stackArray.length - 1 ) {
            ...
            if( !_tokenTaintTracer.has_function( f ) ) {
                ...
                if( f ) {
                    ...
                    try {
                        trace_data.arguments = _tokenTaintTracer.sanitize_arguments( f.arguments );
                    } catch(e){}
                }
    ...
    find_and_log_taint: function ( func, arguments, object_name, function_name ) {
        ...
        if( !_tokenTaintTracer.taint || !(tainted = _tokenTaintTracer.find_taint_in_arguments( arguments )) )
            return;

        _tokenTaintTracer.log_data_flow_sink({
            source:     func,
            function:   func.name || function_name,
            object:     object_name,
            arguments:  arguments,
            tainted:    tainted,
            taint:      _tokenTaintTracer.taint
        });
    },

    find_taint_in_arguments: function( arguments ) {
        for( var i = 0; i < arguments.length; i++ ) {
            var tainted = _tokenTaintTracer.find_taint_recursively( arguments[i] );
    ...
    find_taint_recursively: function( object ) {
        ...
        switch( Object.prototype.toString.call( object ) ) {
            case '[object String]':
                if( object.indexOf( _tokenTaintTracer.taint ) != -1 ) return object;
                break;

            case '[object Array]':
                for( var i = 0; i < object.length; i++ ) {
                    tainted = _tokenTaintTracer.find_taint_recursively( object[i] );
                    if ( tainted ) return tainted;
                }
                break;
                ...
                for( var property in object ){
                    if( object.hasOwnProperty( property ) ) {
                        ...
                        if( Object.prototype.toString.call( property_value ) !== '[object Function]' ){
                            tainted = _tokenTaintTracer.find_taint_recursively( property_value );
                            if ( tainted ) return tainted;
                        }
                    }
                }
