...
# Fuzz with add headers content special char
#
# @author Lionel PRAT <lionel.prat9@gmail.com>
# @version 0.0.1
#
require 'simhash'
class Arachni::Checks::Fuzz_Header_Spechar_Add < Arachni::Check::Base
    ...
    def self.info
        {
            name:        'Strange response on Fuzz with add headers content special char',
            description: %q{
Injects arbitrary special char in new header and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with request content special char in header add},
                description:     %q{
The server response is different than original request.
},
                tags:            %w(response strange special char in header add),
