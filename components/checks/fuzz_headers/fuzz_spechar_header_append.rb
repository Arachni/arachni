...
# Fuzz existence headers with special char (method append)
#
# @author Lionel PRAT <lionel.prat9@gmail.com>
# @version 0.0.1
#
require 'simhash'
class Arachni::Checks::Fuzz_Header_Spechar_append < Arachni::Check::Base
    ...
    def self.info
        {
            name:        'Strange response on Fuzz existence headers with special char (method append)',
            description: %q{
Injects arbitrary special char in existence header and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with request content special char append in headers present},
                description:     %q{
The server response is different than original request.
},
                tags:            %w(response strange special append in headers present),
