...
#
require 'simhash'
class Arachni::Checks::Fuzz_SpecharAppend5c22 < Arachni::Check::Base
    ...
    def self.info
        {
            name:        'Strange response with fuzz request content special char 5c22 (append)',
            description: %q{
Injects arbitrary special char and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with fuzz request content special char 5c22 append},
                description:     %q{
The server response is different than original request.
},
                references:  {
                },
                tags:            %w(response strange special char 5c22 append),
