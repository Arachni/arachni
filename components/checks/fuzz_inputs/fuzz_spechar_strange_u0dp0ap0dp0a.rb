...
#
require 'simhash'
class Arachni::Checks::Fuzz_SpecharStrangeu0d0a0d0a < Arachni::Check::Base

    def self.payloads
        #ADD %c5%unicode (voir code python)
        @payloads ||= [ "\xc4\x8d\xc4\x8a\xc4\x8d\xc4\x8a" ]
    end
    ...
    def self.info
        {
            name:        'Strange response with fuzz request content special char u0d0a0d0a (strange)',
            description: %q{
Injects arbitrary special char and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with fuzz request content special char u0d0a0d0a strange},
                description:     %q{
The server response is different than original request.
},
                references:  {
                },
                tags:            %w(response strange special char u0d0a0d0a strange),
