...
#
require 'simhash'
class Arachni::Checks::Fuzz_SpecharAppend5c0d0a0d0a < Arachni::Check::Base

    def self.payloads
        #ADD %c5%unicode (voir code python)
        @payloads ||= [ "\x5c\x0d\x5c\x0a\x5c\x0d\x5c\x0a" ]
    end
            ...
            if response.timed_out?
                ...
                if response.body.to_s =~ /<title>400 Bad Request<\/title>/i
                    ...
                    log(
                        vector:   element,
                        response: response,
                        proof:    "Response error SIMHASH diff: (simhash #{d_new_body} -- code #{response.code})"+response.to_s
                    )           
                    check_simhash.push(d_new_body)
                    auditedsc( "#{path}::#{response.code}::#{d_new_body}" )
                end
    ...
    def self.info
        {
            name:        'Strange response with fuzz request content special char 5c0d0a0d0a (append)',
            description: %q{
Injects arbitrary special char and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with fuzz request content special char 5c0d0a0d0a append},
                description:     %q{
The server response is different than original request.
},
                references:  {
                },
                tags:            %w(response strange special char 5c0d0a0d0a append),
