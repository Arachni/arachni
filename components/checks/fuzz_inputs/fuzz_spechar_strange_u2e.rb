...
#
require 'simhash'
class Arachni::Checks::Fuzz_SpecharStrangeu2e < Arachni::Check::Base
    ...
    def self.info
        {
            name:        'Strange response with fuzz request content special char u2e (strange)',
            description: %q{
Injects arbitrary special char and checks if return error code, time_out or different content.
            ...
            issue:       {
                name:            %q{Strange response with fuzz request content special char u2e strange},
                description:     %q{
The server response is different than original request.
},
                references:  {
                },
                tags:            %w(response strange special char u2e strange),
