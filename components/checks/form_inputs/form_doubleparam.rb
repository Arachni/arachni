...
class Arachni::Checks::Form_DoubleParam < Arachni::Check::Base

    def run
        print_status 'Verify double input param in form...'
        code_ori = page.code    
        ...
        page.forms.each do |form|
            ...
            form.inputs.keys.each do |key|
                inputs_list[key] = [ form.inputs[key], form.inputs[key]]
            end
            if form.method == :post
            ...
            http.request(action_url, ori_opts) do |res|
                ...
                if (code_new != code_ori) or (d_new_body != d_ori_body)
                    ...
                    if audited?( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::doubleparam" )
                        print_info "Skipping already audited switch method page with code '#{code_new}' at '#{res.to_page.url}'"
                        return
                    end
            
                    audited( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::doubleparam" )
                    
                    log( 
                        vector: form,
                        proof: "Page with double param change result: #{code_new}" 
                    )
                end
            end
        end
