...
class Arachni::Checks::Form_with2meth < Arachni::Check::Base

    def run
        print_status 'Verify double method in form...'
        code_ori = page.code
        ...
        page.forms.each do |form|
            ...
            http.request(action_url, ori_opts) do |res|
                ...
                if (code_new != code_ori) or (d_new_body != d_ori_body)
                    ...
                    if audited?( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::form_doublemeth" )
                        print_info "Skipping already audited switch method page with code '#{code_new}' at '#{res.to_page.url}'"
                        return
                    end
            
                    audited( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::form_doublemeth" )
                    
                    log( 
                        vector: form,
                        proof: "Page with double method change result: #{code_new}" 
                    )
                end
            end
        end
    ...
    def self.info
        {
            name:        'Form with double method',
            description: %q{
It's test to send form with double param.
},
            ...
            issue:       {
                name:            %q{Form with double method},
                description:     %q{
If test passed then it's possible to create uncontrolled context, ...
                ...
                remedy_guidance: %q{
You must verify method accept and reject bad case.
}
            }
        }
    end
