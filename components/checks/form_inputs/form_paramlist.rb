...
=end
# Verify can't send request with name input array []
#
#
...
class Arachni::Checks::Form_ParamList < Arachni::Check::Base

    def run
        print_status 'Verify if accept param array []...'
        code_ori = page.code
        ...
        page.forms.each do |form|
            ...
            form.inputs.keys.each do |key|
                inputs_list[key+"[]"] = form.inputs[key]
            end
            ...
            http.request(action_url, ori_opts) do |res|
                ...
                if (code_new != code_ori) or (d_new_body != d_ori_body)
                    ...
                    if audited?( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::form_listparam" )
                        print_info "Skipping already audited switch method page with code '#{code_new}' at '#{res.to_page.url}'"
                        return
                    end
            
                    audited( "#{res.to_page.url}::#{res.to_page.method}::#{code_new}::form_listparam" )
                    
                    log( 
                        vector: form,
                        proof: "Page with array name in input param change result: #{code_new}" 
                    )
                end
            end
        end
    ...
    def self.info
        {
            name:        'Form with list param',
            description: %q{
It's test to send form with inputs name content array [].
},
...
If test passed then it's possible to create uncontrolled context, ...
                ...
                remedy_guidance: %q{
You must verify taint data type and reject bad case.
}
            }
        }
    end
